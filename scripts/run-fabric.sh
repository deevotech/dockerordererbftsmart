...
# Install chaincode on the 1st peer in each org
   ...
   sleep 100
   # Query chaincode from 1st peer of the 1st org
   initPeerVars ${PORGS[0]} 1
   ...
   peer chaincode invoke -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["invoke","a","b","10"]}' $ORDERER_CONN_ARGS
   #sleep 5
...
function chaincodeQuery {
   ...
   while test "$(($(date +%s)-starttime))" -lt "$QUERY_TIMEOUT"; do
      sleep 1
      peer chaincode query -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["query","a"]}' >& log.txt
