...
function writeOrderer {
   ...
   echo "  $ORDERER_NAME:
    ...
    image: hyperledger/fabric-ca-orderer-bftsmart
    environment:
...
function writePeer {
   ...
   echo "  $PEER_NAME:
    ...
    environment:
      ...
      - CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=bftsmartnetwork_${NETWORK}
...
function writeReplica {
for i in 0 1 2 3; do
echo "  replica-$i:
    working_dir: /opt/gopath/src/github.com/hyperledger/hyperledger-bftsmart-release-1.1
    image: bftsmart
    command: /bin/bash -c '/scripts/start-replica.sh 2>&1 | tee /data/logs/replica-$i.log'
    environment:
      - NUMBER=$i
    volumes:
      - ./scripts:/scripts
